...
def encode_data(sentence_file, vocab):
  ...
  for g in get_n_grams(sentence_file):
    inputs.append(get_one_hot(g[0].lower(), vocab) + get_one_hot(g[1].lower(), vocab) + get_one_hot(g[2].lower(), vocab))
    labels.append(get_one_hot(g[3].lower(), vocab))
  return LabeledData(inputs, labels)

def get_test_validation_train(data, test_size, validation_size):
  return LabeledData(data.inputs[:test_size], data.labels[:test_size]), LabeledData(data.inputs[test_size:(test_size + validation_size)], data.labels[test_size:(test_size + validation_size)]), LabeledData(data.inputs[(test_size + validation_size):], data.labels[(test_size + validation_size):])
